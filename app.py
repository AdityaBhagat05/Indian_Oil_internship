...
#         self._mode       = "Monthly"      # track Daily / Monthly selection
...
#         # ── Card 2 — Daily / Monthly mode selector ────────────────────────────
#         c_mode = _card(page)
#         c_mode.grid(row=2, column=0, sticky="ew", padx=32, pady=(14, 0))
#         c_mode.grid_columnconfigure(0, weight=1)

#         mh = ctk.CTkFrame(c_mode, fg_color="transparent")
#         mh.grid(row=0, column=0, sticky="w", padx=22, pady=(18, 12))
#         _badge(mh, "2").grid(row=0, column=0, padx=(0, 10))
#         ctk.CTkLabel(mh, text="Select Injection Mode",
#                      font=ctk.CTkFont(size=14, weight="bold"),
#                      text_color=C_TITLE).grid(row=0, column=1)

#         seg_frame = ctk.CTkFrame(c_mode, fg_color="transparent")
#         seg_frame.grid(row=1, column=0, sticky="w", padx=22, pady=(0, 18))
...
#         self._mode_seg.grid(row=0, column=0)

#         self._mode_desc = ctk.CTkLabel(
#             seg_frame,
#             text="📅  Uses lookup_monthly.py — injects monthly formulas",
#             font=ctk.CTkFont(size=12),
#             text_color=C_MUTED, anchor="w",
#         )
#         self._mode_desc.grid(row=1, column=0, sticky="w", pady=(8, 0))

#         # ── Card 3 — Month + Year ─────────────────────────────────────────────
#         c2 = _card(page)
#         c2.grid(row=3, column=0, sticky="ew", padx=32, pady=(14, 0))
#         c2.grid_columnconfigure(0, weight=1)
#         c2.grid_columnconfigure(1, weight=1)
...
#         mhdr.grid(row=0, column=0, sticky="w", pady=(0, 8))
#         _badge(mhdr, "3").grid(row=0, column=0, padx=(0, 8))
#         ctk.CTkLabel(mhdr, text="Month",
#                      font=ctk.CTkFont(size=14, weight="bold"),
#                      text_color=C_TITLE).grid(row=0, column=1)
...
#         yh = ctk.CTkFrame(yf, fg_color="transparent")
#         yh.grid(row=0, column=0, sticky="w", pady=(0, 8))
#         _badge(yh, "4").grid(row=0, column=0, padx=(0, 8))
#         ctk.CTkLabel(yh, text="Year (YY)",
#                      font=ctk.CTkFont(size=14, weight="bold"),
#                      text_color=C_TITLE).grid(row=0, column=1)
...
#             command=self.start_process,
#         )
#         self.run_btn.grid(row=4, column=0, sticky="ew", padx=32, pady=(16, 0))

#         # ── Activity log card ─────────────────────────────────────────────────
#         c3 = _card(page)
#         c3.grid(row=5, column=0, sticky="ew", padx=32, pady=(16, 32))
#         c3.grid_columnconfigure(0, weight=1)
...
#                 text="📆  Uses lookup_daily.py — injects daily formulas"
#             )
#         else:
#             self._mode_desc.configure(
#                 text="📅  Uses lookup_monthly.py — injects monthly formulas"
...
#             self.log_message("⚠  Month and Year are required.")
#             return
#         mode_label = self._mode          # "Daily" or "Monthly"
#         self.run_btn.configure(state="disabled",
#                                text=f"⏳   Injecting ({mode_label})…",
#                                fg_color="#9CA3AF", hover_color="#9CA3AF")
#         threading.Thread(
#             target=self._worker, args=(month, year, mode_label), daemon=True
#         ).start()

#     def _worker(self, month: str, year: str, mode: str):
#         def gui_log(msg):
#             self.after(0, self.log_message, msg)
...
#             gui_log(f"⚠  {module}.py not found — running in demo mode.")
#             gui_log(f"✅  Would inject {mode.lower()} formulas for {month}{year} "
#                     f"into {os.path.basename(self.target_file)}")
#         except Exception as e:
#             gui_log(f"❌  Error: {e}")
#         finally:
...
class FormulaInjectorApp(ctk.CTk):
    ...
    def _build_help(self, parent) -> ctk.CTkScrollableFrame:
        ...
        cc.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(cc,
                     text="Still stuck? Reach out at https://github.com/AdityaBhagat05/Indian_Oil_internship",
                     font=ctk.CTkFont(size=13), text_color=ORANGE,
                     ).grid(row=0, column=0, padx=22, pady=16, sticky="w")
    ...
    def _worker(self, month: str, year: str, mode: str):
        ...
        try:
            ...
            module = "lookup_daily" if mode == "Daily" else "lookup_monthly"
            gui_log(f"Warning: {module}.py not found - running in demo mode.")
            gui_log(f"Would inject {mode.lower()} formulas for {month}{year} "
                    f"into {os.path.basename(self.target_file)}")
        except Exception as e:
            gui_log(f"Error: {e}")
